---
sidebar_position: 0
title: Ways of Working
---

# Ways of Working

Los acuerdos de proceso que definen cómo trabaja el equipo día a día. Desde cómo planificamos hasta cómo manejamos deuda técnica y compartimos conocimiento.

Estos no son reglas impuestas — son decisiones colectivas que el equipo ha tomado para trabajar mejor juntos.

---

## Proceso & Planificación

Cómo organizamos el trabajo, estimamos esfuerzo y definimos cuándo algo está listo.

- [Scrum & Agile Model](./scrum) — el modelo de trabajo iterativo del equipo
- [Estimación](./estimacion) — cómo estimamos historias y tareas
- [Slicing de Historias](./slicing) — cómo partimos trabajo en piezas entregables
- [Definition of Ready / Done](./definition-of-ready-done) — criterios de entrada y salida de una historia

---

## Calidad & Mantenimiento

Cómo priorizamos lo que falla y lo que acumulamos.

- [Priorización de Bugs](./priorizacion-de-bugs) — niveles de severidad y tiempos de respuesta
- [Tech Debt](./tech-debt) — cómo identificamos, registramos y pagamos deuda técnica

---

## Cultura & Conocimiento

Espacios para crecer y compartir como equipo.

- [Tech Talks](./tech-talks) — formato y frecuencia de las charlas técnicas internas
